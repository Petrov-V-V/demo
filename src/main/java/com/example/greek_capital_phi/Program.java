...
public class Program {
	public static double listTask() {
		...
		return integerList.stream().limit(integerList.size() - 3).mapToInt(a -> a).map(a -> a % 2 == 0 ? a + 5 : a - 5)
				.average().getAsDouble();
	}

	public static Map<String, String> setTask() {
		Set<String> stringSet = Set.of("zero", "one", "two", "three", "four", "twenty-three", "thirty-four",
				"seventy-seven", "eighty-eight", "one hundred");
	...
	public static List<String> mapFirstTask() {
		...
		return stringMap.entrySet().stream().map(a -> a.getKey() + " == " + a.getValue()).collect(Collectors.toList());
	}
